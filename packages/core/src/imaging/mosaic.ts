...
export interface MosaicTile {
  /** Column and row in the mosaic, `0`-based from −x, −y. */
  readonly col: number;
  readonly row: number;
  readonly frame: ObjectFieldFrame;
  /** Pixel offset of this tile's kept span within the composed image. */
...
export function mosaicLayout(system: OpticalSystem, options: MosaicOptions): MosaicLayout {
  const { tiles, size, pupilSamples, guardCells } = options;
  if (!Number.isInteger(tiles) || tiles < 1) {
    throw new Error(`mosaicLayout: tiles must be a positive integer, got ${tiles}`);
  }
  if (!(guardCells >= 0)) {
    throw new Error(`mosaicLayout: guardCells must be >= 0, got ${guardCells}`);
  }
  ...
  if (!Number.isInteger(guardPixels)) {
    throw new Error(
      `mosaicLayout: a guard of ${guardCells} cells is ${guardPixels} pixels at size ${size} ` +
  ...
  if (usefulPixels < 1) {
    throw new Error(
      `mosaicLayout: a guard of ${guardCells} cells eats the whole ${size}-pixel tile ` +
        `(${guardPixels} px per edge leaves ${usefulPixels})`,
    );
  }

  const centre = options.centreMm ?? { x: 0, y: 0 };
  const base = { size, pupilSamples } as const;
  const common = {
    ...base,
...
export function renderMosaic(
  ...
  const { usefulPixels, guardPixels, size } = layout;
  ...
  for (const tile of layout.tiles) {
    ...
    for (let y = 0; y < usefulPixels; y++) {
      ...
      for (let x = 0; x < usefulPixels; x++) {
        intensity[dst + x] = formed.intensity[src + x]!;
      }
    }
