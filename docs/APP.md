...
be right at both ends of one frame out there. **§ 6bg adds a warning the panel
must carry if it shows the haze number at all**: `inFocusFraction` is measured
about the NOMINAL stage, and a focus-corrected render puts the specimen a whole
chromatic shift away from it — so the corrected frame, the one 5.88× brighter at
its peak, reads **zero** in focus while the blurred uncorrected one reads one
(§ 6bg.5). The haze number answers where a thick specimen's light comes from
relative to the stage; it is not a sharpness meter and a panel that puts it
beside a focus control will be read as one. Still absent: `hazeKernel`,
...
| ~~Polychromatic brightfield~~ / fluorescence colour | ~~§ 6f and § 6i both name it open. § 6j's band is emission-only. Scoped as § 6r in Part D.~~ **Brightfield colour unblocked at § 6r** — `brightfieldSpectralStack` runs the Abbe sum per wavelength and `colorImageFromStack` collapses it. ~~Fluorescence colour is still open~~ — **unblocked at § 6ba**: `fluorescenceSpectralStack` renders a `SpectralEmitterDensity` one plane per wavelength and `colorImageFromStack` collapses it, with `channelBasis` giving a per-filter channel off the same exposure. Neither half is engine-blocked now; what is missing is a **panel**, and it is a Part Q variant rather than a new surface — the emitter page plus a label list (density + band) and a filter, at that page's measured cost times the wavelength count. A **thick** preparation is unblocked too as of § 6bb — same page plus a slab list — and it costs the wavelength count times the slice count, with the honest warning § 6bb.6 supplies: the channels do not focus at the same stage position, so a focus control is per-exposure and not per-channel. **§ 6be adds the field to that warning and the patches to the panel**: the thick variant now takes `patches`, so the field decomposition above is available in colour; and best focus moves across the field as well as across the band, by half as much again (0.250229 mm over the square against 0.166910 mm across the band alone), so one stage position is wrong in two directions at once. The page should say that rather than imply a focus control can be right anywhere but at one field radius — and § 6be.2 says the correction it would need is two one-dimensional curves and not a map. **§ 6bf returns those curves**: `focusSurface` sweeps the stage per wavelength and per field height and hands back the sampled curves plus `interactionMm`, the figure the two-curve prediction is worth. A panel may show them — at the price of the sweep, which is a rendered stack per grid point and not a readout on an existing frame — but it may not ship a stored coefficient: § 6bf.4 measures the curve differing between objectives in shape as well as scale, so the curve belongs to the objective the user has chosen and has to be measured for it. **§ 6bg renders with the curve**, so the focus control this row has been circling is now buildable — with two things the page has to say beside it. It is **several exposures and not one**: `exposures` reads 3 for a per-filter step and 9 for a per-node curve, and `focusMm` goes `undefined` because there is no single stage the picture was taken at, so anything that moves or bleaches between them is registered wrong. And it **costs registration** — blue against red grows 22.2% further apart at 1.0 mm of field (§ 6bg.6), for a reason that is not the perspective term § 6bb.10 bounded — so a per-channel focus control and a two-stain overlay measurement do not belong on one page without the page saying which runs first. A per-tile control is the other half, and ~~it is a **tile series**, not a mosaic: § 6bg composes nothing, so the panel would show tiles side by side and not a stitched field~~ — **§ 6bh composes them**, so the panel can show one stitched field: `renderFluorescenceMosaic` takes the tile count, a guard in resolution cells and the same focus surface, and hands back a composed stack `colorImageFromStack` accepts unchanged. Two things the page has to carry with it. The **guard band is a real control and its default is not obvious** — § 6bh.4 measures what it has to contain, and it is the *stage error* rather than the specimen: an uncorrected 430 nm tile spills 9.82% of a point's light past its own frame against a corrected one's 1.87%, while going from a 0.016 mm specimen to a 0.24 mm one adds 0.26 points. So a page that offers the correction and the mosaic together should not also offer an uncorrected mosaic without saying that its tiles are outside the containment limit the engine reports (`maxGridPhaseStepWaves` past half a wave, § 6bd.8). And the **seams are visible steps** — nothing is blended unless the caller asks — carrying a focus step that is a field quantity: 0.004 of a depth of focus across an axial seam against 0.159 at 1 mm of field (§ 6bh.5). A viewer showing the stitched field at the edge of the catalogued field will see that staircase, and it is the picture being honest rather than a rendering artefact. **§ 6bi says the page has been warning about the wrong seam.** The focus step is invisible on a featureless region — a uniform object is uniform under any kernel — while the objective's own transmission puts a **brightness staircase** there: 5.29e-3 across a seam at 1 mm of field against 1.90e-6 on the axis (§ 6bi.2), which is what a viewer actually sees as a grid. It is correctable and the correction is nearly free: `throughputFlatField` builds a field out of `patchThroughput`, which every render already carries, and `flatFieldCorrect` divides — 121× off the seam for no extra render (§ 6bi.4). `renderedFlatField` is the exact version and costs a second whole mosaic, so a panel should offer the free one by default and the blank-slide calibration as an explicit second acquisition. Two warnings go with it. A flat field is a **division**, so it reaches brightness and not sharpness — 304× more of the first than the second on a point emitter — and the focus step stays exactly where § 6bh.5 measured it. And the page must **not** offer the correction a real slide scanner uses — one calibration frame repeated in every tile — because this mosaic scans the objective's field where a scanner scans the stage, and repeating a frame makes the seam **11% worse** (§ 6bi.4). An `overlapPixels` control exists too and is honestly cosmetic: it divides the *visible* step by the overlap and leaves the total alone (§ 6bi.5), it costs `(1 − f)^−2` more tiles at overlap fraction f, and its optical price is that a blended pixel averages two blurs — 2.3% of the second moment at the field edge, where the double-image term one would fear is 1.6e-6 of it (§ 6bi.6). Offer the flat field before the blend. Cost is the tile render's times tiles², with no economy between tiles — a per-tile stage gives every tile its own raster — so a 3×3 at the emitter page's measured per-frame cost is nine of them. **§ 6bj makes the acquisition geometry a control, and it is the one choice on this page that moves every other number on it.** The default walks the frame across the objective's field; `scan: "stage"` holds the objective still and moves the slide, which is what a real scanner does. Under the stage scan every tile is formed at one field position, so the page gets *one* of everything: one stage (the focus control goes back to being per channel and stops being per tile, and the nine tiles' stages are bitwise equal), one flat field (the free `throughputFlatField` goes inert — span exactly 0 — so the blank-slide calibration is no longer the expensive option but the only one that does anything), and one kernel (the guard-band warning above becomes a single number rather than nine). The seam artifacts this row has been warning about largely go with it: the brightness step falls 7.6× and the focus step to exactly zero (§ 6bj.3, § 6bj.4). **What it costs is registration, and the page must say so**: a square stage lattice cannot abut a radial map in both directions at once, so at the edge of the field a seam carries **0.33 px** against a field scan's 0.003 — 95.7× — and 40.8× more across the scan than along it (§ 6bj.5). **That 95.7× is per-FRAME and not per-objective** — § 6bo measures a tile's field of view to be ∝ M/NA at a fixed `pupilSamples`, and the same 20× objective reads 68.8×, 21.0× or 8.2× at three frame sizes, so a panel quoting a registration cost has to quote the frame it was measured on. § 6bo.4 withdraws the "95.7 over M" budget § 6bl.4 offered outright: at a matched field the cost RISES with magnification. So the honest framing for a control is *uniform against continuous*: pick the stage scan when the tiles have to be comparable to each other, the field scan when the picture has to stitch into one. And under a stage scan the mosaic's centre is the whole quality choice — anchored on the axis it is the best tile everywhere, anchored at the edge it is a mediocre tile everywhere (§ 6bj.6) — so a viewer that lets the user drag the mosaic around the field is changing image quality, not just the view. Cost is unchanged: the renderer deliberately takes no economy from the shared field position, so a stage scan is still one tile render per tile. |
...
- ~~**No traced targets *here*.**~~ ✅ **landed — two of the engine's five
  ...
  **Whole-readout cost over every offered cell**, on this machine under
  ...
  paragraph first found it. **The wavefront corner said "no per-ray form yet and
  NOT fixed" until § 1.8.13, and it is now fixed too** — by a different remedy
  than the one § 1.8.12 forecast for it, because a wavefront RMS is already a
  root-sum-square and its rows are the Zernike coefficients rather than the
  samples. Asked that way, the shipped on-axis cell converges on `step` in 195
  traced evaluations against the 2 001 cap and reads **6.842737e-3 waves against
  9.122502e-3** — 33.3% better — and the 313-ray off-axis corner this paragraph
  names goes from 2 001 evaluations at 1.781354e-1 waves to 105 at
  **2.900004e-2**, which is **6.1× better**, and from ~3.5 s to **0.83 s**. The
  off-axis gain is the larger one for the reason the corner was slow in the
  first place: two curvatures cannot correct field coma, so the one-row model's
  blind direction is where most of the error lives. A reader comparing the two
  traced readings on this panel is now comparing two converged answers, and the
  sentence this replaces is kept in outline because the round trip is the
  finding — the panel shipped a stuck wavefront number for as long as it shipped
  a stuck spot one, and each was only visible once the other was fixed. The same sweep re-run reads 51 ms to 5.5 s, so
